...
export interface Provides {
	 ...
	 * every name two labels below it (D-next). `at: dash` in a Launchfile is
...
export interface Commands {
	...
	/**
	 * Source-mode prepare — install deps / codegen when running from source (D-38).
	...
	/**
	 * Source-mode run — run the app from source (D-38). The source-mode pair of
...
export interface Component {
	...
	/** Build configuration */
	build?: string | Build;
	/** Working directory for source-mode commands (install/dev); defaults to build.context, then repo root (D-38) */
	source?: string;

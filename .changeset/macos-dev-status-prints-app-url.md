...
With an `appUrl` set — on this run or recorded by an earlier one — the "is running at" summary and the `status` "Components:" list show that URL, as stored, on the primary component: the one `$app.*` reads (a declared `https-origin` component, else the first with an `exposed: true` endpoint), when its primary endpoint is `http` or `https`. A `ws`, `tcp`, `udp` or `grpc` primary keeps this provider's own address, since the URL says nothing about what that listener speaks (D-58 rule 2). Every other component keeps this provider's own `http://localhost:<port>` (D-58 rule 4). With no URL supplied, output is unchanged.

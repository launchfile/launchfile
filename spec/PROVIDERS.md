...
**Publication context (`$app.*`) under an owning orchestrator.** `$app.*` stays home #3 — the provider computes it from its routing strategy (D-33, D-35, D-36). When the provider runs embedded under an orchestrator that owns routing — a reverse proxy, tunnel, or edge in front of it — the routing strategy has moved upstream, and the provider's own answer (e.g. `http://localhost:<port>`) is no longer the app's public address. In that operation the provider resolves `$app.*` from the public URL the orchestrator supplies via the provider's **orchestrator-facing channel** (`@launchfile/docker`: `ComposeOpts.appUrl`), the same way §10 rule 8 has a `required:` value arrive through the operator-facing channel — and it MUST refuse a malformed value rather than degrade, guess, or fall back to its own routing answer. The supplied URL asserts the public address of the app's **primary** endpoint only; a provider MUST NOT derive other published endpoints' public addresses from it (per-endpoint publication context is a future proposal). Unset, the provider's own routing strategy stands, unchanged. Two reference providers have no such channel today and are out of scope for `ComposeOpts.appUrl`. `@launchfile/macos-dev` resolves `$app.*` from its own routing strategy only ([#294](https://github.com/launchfile/launchfile/issues/294)). `@launchfile/aws` composes `$app.*` from a Terraform load-balancer reference (`${aws_lb.main.dns_name}`, `providers/aws/src/translate.ts:219`) that no caller can supply, because the address does not exist until apply time; a publication-context channel there is a separate proposal ([#304](https://github.com/launchfile/launchfile/issues/304)), not an extension of this one.

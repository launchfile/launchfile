...
| `$app.endpoints.<name>.*` — `url`, `host`, `port`, `scheme`, `authority`, `tls` | the public address of the named published endpoint, from the same derivation as `$app.*`; `""` where the provider publishes no per-endpoint address | **D-next** |

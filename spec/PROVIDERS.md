...
| `dev`                | not implemented (source-mode run slot D-38 lets an artifact-executing provider ignore) | no resolution — raw to the process manager                            | no resolution — raw into the systemd unit |
...
**What to do meanwhile.** Nothing here licenses `$` in a command: the same expression resolves on one provider and slot and vanishes on another, which is exactly the P-5 breakage this document exists to prevent. **There is also no portable workaround.** Routing the value through an `env:` variable and reading it back as a shell variable works on every slot that does *not* resolve — but on the slots that do (docker `bootstrap`, docker `release`, macos-dev `bootstrap`), resolution covers the whole string and blanks the `$VAR` too: `catalog/apps/paperclip`'s bootstrap is destroyed on `@launchfile/docker` today for precisely this reason. Until #205 lands, read the table for the provider and slot you actually target, and expect neither `$` form to be portable.

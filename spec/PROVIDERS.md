...
8. **Report gaps, not silent drops** — if a field can't be honored, surface it (the AWS probe's conformance report is the model). Two fields carry the hard form of this rule:
   ...
   - **`env.<NAME>` with `required: true`** — a provider **MUST NOT** substitute a **fabricated** value for an **unsupplied** required environment variable (D-44). Both terms are narrow:
     ...
     - **Deploying verbs** (`up`, and any verb that provisions or runs) — for a component it is launching, a provider MUST either obtain the value from its own operator-facing channel (the launching environment, a prompt, a secret store; which one is the provider's choice, P-11/L-3) or **fail**, naming the component and the variable. Component selection bounds which components this reaches (§5, rule 4): an unsupplied variable in a component outside the start-set is not a launch-blocking gap.
     ...
     - **`down`, `list`, and `logs`** — no obligation. Teardown resolves nothing, `list` enumerates deployments, and `logs` streams process output (§2); none of the three resolves or emits environment values, so an unsupplied variable is not reachable through them.
     ...
     **Adoption status:** at the time this rule was written, the deploying branch is satisfied by **no** reference provider — docker fabricates and macos-dev silently omits (both are being fixed); aws satisfies the `translate` branch. A provider author reading this section should treat the rule as normative and the reference implementations as catching up, not the reverse. See D-44 → *Conformance at adoption*.

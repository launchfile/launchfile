...
2. **Absent means today. Present means exactly this.** A file with no `at:` keeps today's meaning. An entry that declares `at:` answers at the listed names and no others. Without `"@"` in the list the app does not answer at the app host, and the platform may put anything there.
...
5. **Provision or refuse.** For each value a provider either provisions the names — requests for a matching name reach this entry's listener with the requested host in `Host`, DNS resolves the name, and the provider serves a certificate valid for the name where it terminates TLS — or **refuses the component before launch**, naming the entry and the value it cannot cover. There is no partial coverage. A provider that cannot form a valid name also refuses: an app host that is an IP address, or a name longer than 253 octets. An orchestrator that owns routing and supplies the public URL provisions these names itself, and a provider never infers from that URL that a value is covered; until a channel exists for the orchestrator to state which values it covers ([#543](https://github.com/launchfile/launchfile/issues/543)), a provider under a supplied URL refuses too. See [PROVIDERS.md](PROVIDERS.md) §7 and §10.
...
Every declared value is required: `at:` has no optional mood. A reader that predates `at:` ignores the field and publishes one host, which is today's result for such an app, not a new one.

...
1. **Minted** — `generator:` present. The platform generates the value **once**, then preserves it. A `generator:` satisfies `required:` (miniflux's `ADMIN_PASSWORD`: `required` + `generator` classifies minted).
...
- **Author default** → ordinary per-environment config the orchestrator may override (exactly [D-36](#d-36-the-three-homes-of-a-varying-value-p-1-litmus-refinement) home #2 / [L-3](#l-3-no-environment-specific-overrides)). A platform fixing snipe-it's literal `"http://localhost"` on a real deployment is doing its job.

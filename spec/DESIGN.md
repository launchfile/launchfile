...
### D-36: `install` / `dev` — source-mode lifecycle commands

**Decision**: Add two well-known command keys — `install` (source-mode *prepare*) and `dev` (source-mode *run*) — plus one optional component field, `source:` (the working directory for source-mode commands). Together they let a Launchfile run **from source**, not just as a built artifact, per the execution-mode taxonomy ([#77](https://github.com/launchfile/launchfile/issues/77)): execution mode (source vs. artifact) varies the *commands*, and **only the prepare and run steps are mode-aware**. `install` is the source-mode prepare (e.g. `bun install`), paired with artifact-mode `build`; `dev` is the source-mode run (e.g. `bun run dev`), paired with artifact-mode `start`. `release`, `bootstrap`, `seed`, and `test` are **mode-invariant** — the same command runs in both modes; a path or binary that differs by mode lives in `storage:` / `env` / the provider `PATH`, not a separate command. `source:` defaults to `build.context`, then the repo root. The keys need no schema change — `CommandsSchema` is an open record (P-13); `source:` is one optional additive field.

**Resolution** (per component): a provider selects one mode for the app (`launchfile dev` → source, `launchfile up` → artifact) and resolves each component. Run precedence is **`dev` > `image` > `start`** — `dev` runs the component from source; an `image` keeps it in artifact mode unless `dev` overrides it; a bare `start` runs from source only when there is no `image`, so a prebuilt image is never replaced by a `start` that assumes the image's internals (fallbacks must be detectably safe, #77). Source-mode prepare is `install ?? build`, run **on demand** (first launch or a detected dependency change), not on every run. Component **selection** is a verb argument (`launchfile dev backend`), not a field.

**Rejected**:
- **The compound `dev:<stage>` form** (`dev:build` / `dev:release` / `dev:bootstrap` / `dev:seed` / `dev:test`, the original #45 proposal) — it makes *every* stage mode-aware, but only prepare and run diverge by *intent* (#77); a `release`/`bootstrap`/`seed`/`test` difference is a path or binary-location difference that belongs in `storage:` / `env` / `PATH`, not a per-stage command variant. Flat keys also map 1:1 to `package.json` scripts; the compound form makes two colons do two jobs (mode × stage).
- **Reusing `commands.start` for both contexts** — overloading one field makes the file correct in only one mode (a compiled-binary image starts via its ENTRYPOINT while the source tree starts via `bun run dev`).
- **A `profiles:` / `environments:` override block** — the full L-3 per-environment mechanism (merge semantics, precedence, unbounded scope). Mode is not environment (#77); the carve-out is scoped to the two mode-aware command keys.
- **Provider-named sections** (`macos:`, `docker:`) — violates P-5; "dev" is a mode of execution, not a provider name.
- **A separate `Launchfile.dev` file** — splits the source of truth (P-10).
**Why**: The format commits to "same file, different translators" for dev/prod parity (12-Factor X). The translation diverges at the command lines, because source trees and built artifacts expose different entrypoints. Naming the two mode-aware commands (`install`, `dev`) captures that divergence while every other field stays shared. Aligns with P-1 (the from-source workflow is app knowledge — it lives in `package.json` today — and varies by execution mode, not deployment target), P-8 (mirrors the near-universal `package.json` `"dev"` script), P-11 (intent vs. execution: the app declares its source commands, the provider selects the mode), and P-13 (additive — open-record keys + one optional field; removal breaks no file). Supersedes [#45](https://github.com/launchfile/launchfile/issues/45); see RFC B ([#79](https://github.com/launchfile/launchfile/issues/79)) for the proposal and #77 for the execution-mode taxonomy it rests on.
...
**Current stance**: The resolver code in the SDK is the de facto specification. Resolution order is: (1) `app.*` reserved platform-injected properties (D-33), (2) `secrets.*`, (3) `components.*`, (4) single segment from enclosing resource, (5) multi-segment as named resource lookup, (6) fallback to enclosing resource with dotted key.
...
**Current stance**: Environment-specific values are orchestrator concerns. The orchestrator resolves env vars differently per environment. Execution mode (source vs. artifact) is a *distinct* axis (D-36, [#77](https://github.com/launchfile/launchfile/issues/77)): the `install` / `dev` source-mode commands are app-knowledge command variants, not per-environment overrides, and "dev" is a mode, not an environment.

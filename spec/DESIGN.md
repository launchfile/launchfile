...
**Conformance at adoption** (originally recorded 2026-08-25 with the channel unimplemented; updated 2026-08-26 after the channel shipped — the original wording is preserved in git history): the *format* carries the marker — schema key, SDK round-trip, the `validate` privilege-summary line, and the `persistent: false` contradiction warning. Of the three surfaces the original paragraph named, two have since learned the channel: **`providers/docker`** implements the grant/refuse contract (bind mount on a supplied path, loud refusal otherwise), and **`catalog/test/src/launch-to-compose.ts`** accepts `storagePaths` and reports `storageRefusals`. The CLI spelling `up --storage <volume>=<path>` shipped with them, and the `getPositional` defect on its critical path is fixed — `packages/launchfile/src/cli-args.ts` carries a `VALUE_FLAGS` table so flag values are never consumed as positionals ([#248](https://github.com/launchfile/launchfile/issues/248), closed). Still open: **`providers/macos-dev`**'s `provisionStorage` retains its unconditional `mkdir` and has not gained rule 2's refuse branches — tracked in [#296](https://github.com/launchfile/launchfile/issues/296). Catalog adoption of the marker continues to wait on that surface. What is deferred is the work, not the obligation.

...
### D-35: Dev-mode command variants (`dev`, `dev:<stage>`)

**Decision**: Introduce `commands.dev` as the dev-mode variant of `start`, and a `dev:<stage>` key prefix (`dev:build`, `dev:release`, `dev:bootstrap`, `dev:seed`, `dev:test`) as dev-mode variants of the other lifecycle stages. `dev` is the only mode prefix the spec defines (environment names are explicitly excluded — L-3), and `dev:start` is not a recognized key (bare `dev` is the one obvious way). Dev-mode providers (those that run the app from source, e.g. the macOS dev provider) resolve each stage as `dev:<stage> ?? <stage>` (with bare `dev` standing in for `dev:start`). Providers that execute the built artifact (Docker, Kubernetes, cloud platforms) MUST ignore `dev` and all `dev:*` keys.
**Rejected**:
- **Reusing `commands.start` for both contexts** — the artifact's start command and the source tree's dev command are routinely different programs. A compiled-binary image starts via its ENTRYPOINT (`start` is often absent), while the source tree starts via `bun run dev`. Overloading one field forces the file to be correct in only one mode — exactly the failure observed with the claudewerk broker, where the native dev command (`bun src/broker/index.ts`) would be spliced into a container whose runtime image doesn't contain bun.
- **A `profiles:`/`environments:` override block** — the full per-environment override mechanism (L-3) is a much larger design with merge semantics, precedence rules, and unbounded scope (any field could be overridden). Dev-mode commands are the single highest-frequency divergence, and scoping the carve-out to command strings keeps the addition flat, greppable, and additive.
- **Provider-specific sections** (`macos:`, `docker:` keys) — violates P-5 (provider-translatable): the file would enumerate providers instead of declaring intent. "Dev mode" is a mode of execution (from source vs. from artifact), not a provider name; any number of dev-mode providers can honor the same keys.
- **A separate `Launchfile.dev` file** — splits the source of truth (P-10) and drifts; the dev command is part of the same app's description.
**Why**: The format already commits to "same file, different translators" for dev/prod parity (12-Factor X). In practice the translation diverges at exactly one point: the command lines, because source trees and built artifacts expose different entrypoints (`bun run dev` vs. a compiled binary; a repo-local `bin/cli` vs. a CLI on the image PATH; a project-local data dir vs. a container volume path). `dev:*` names that divergence explicitly while leaving every other field shared. No schema change is required — `CommandsSchema` is already an open record (P-13), so existing parsers tolerate the keys; the spec change is documentation of well-known names plus the resolution rule. The `dev` shorthand mirrors the near-universal `package.json` `"dev"` script convention, so generated Launchfiles read naturally to developers.
...
**Current stance**: Environment-specific values are orchestrator concerns. The orchestrator resolves env vars differently per environment. D-35's dev-mode command variants (`dev`, `dev:<stage>`) are a deliberately narrow carve-out for the run-from-source case — they do not generalize to staging/production overrides.

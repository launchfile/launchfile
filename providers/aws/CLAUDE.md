...
- `prior-stack.ts` — reads the output dir for the `random_*` types already minted (`terraform.tfstate` when it parses, else `main.tf`), so a re-translate preserves a secret instead of rotating it (D-49, D-69). Types and names only — never a value
